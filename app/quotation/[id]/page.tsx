    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8">
      ...
      <GlassCard variant="warm" className="p-8 md:p-12 space-y-8 border-2 border-gold-400 shadow-2xl">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-gold-200">
                {cartLines.length === 0 ? (
                  <tr>
                    <td className="p-3 text-maroon-800" colSpan={3}>No items selected yet.</td>
                  </tr>
                ) : (
                  cartLines
                    ...
                    .map((line) => (
                      <tr key={line.id}>
                        ...
                        <td className="p-3 text-right text-maroon-900 font-bold">{formatCurrency(line.unitPrice * line.quantity)}</td>
                      </tr>
                    ))
                )}
                {requestedExtras.map((line) => (
                  <tr key={line.id}>
                    ...
                    <td className="p-3 text-right text-maroon-800">&mdash;</td>
                  </tr>
                ))}
                <tr className="border-t-2 border-maroon-800">
                  <td className="p-3 font-bold text-maroon-950" colSpan={2}>Estimated Total</td>
                  <td className="p-3 text-right font-bold text-maroon-950 text-sm">{formatCurrency(estimatedTotal)}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div className="bg-maroon-900 text-ivory p-6 rounded-2xl border-2 border-gold-400 text-center space-y-1">
          <p className="text-sm font-bold text-gold-300">This Is An Estimate, Not A Final Invoice</p>
          <p className="text-xs text-gold-100/80">
            Request a quote and our team will confirm final pricing and availability based on your selections.
          </p>
        </div>
